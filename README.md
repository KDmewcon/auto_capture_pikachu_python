...
- Calibrate luoi bang 2 o moc (tam o tren-trai va tam o duoi-phai) de chia o chinh xac
...
4. Mo cua so calibrate luoi: click tam o goc tren-trai, sau do click tam o goc duoi-phai
...
- `G`: Calibrate lai luoi o (2 diem moc)
...
Bat buoc calibrate lai luoi:

```bash
python app.py --select-region --select-grid
```
...
```bash
python app.py --monitor 1 --select-region --select-grid \
	--similarity 0.93 --ambiguity-margin 0.02 --template-update 0.16
```
...
- File `pikachu_config.json` luu them `board_grid` (offset + kich thuoc luoi trong ROI) de click on dinh hon.
